    ...
    <div ref={mainRef} className="relative overflow-hidden">
      ...
      <section className="relative h-dvh w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center flex flex-col items-center justify-center h-full px-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            <Button variant="secondary" className="shadow-2xl" onClick={(e) => handleAddToCart({ id: "p1", name: "Radiance Serum", price: 2499, image: "/images/serum.png" }, e)}>Shop Bestsellers — ₹1,299 Onwards</Button>
      ...
      <section className="reveal-section py-5 bg-white overflow-hidden">
        <div className="container mx-auto px-4 md:px-12">
          ...
          >
            {[
              { id: "p1", name: "Radiance Serum", price: 2499, mrp: 3499, cat: "🔥 #1 Bestseller", image: "/images/serum.png", hook: "The dark spots you've been hiding? They start fading in 21 days.", rating: "4.9", reviews: "2.3K" },
              { id: "p2", name: "Barrier Cream", price: 1899, mrp: 2699, cat: "⭐ 4.9 Stars", image: "/images/cream.png", hook: "Waking up to flaky, tight skin? This ends that. Permanently.", rating: "4.8", reviews: "1.8K" },
              { id: "p3", name: "Botanical Oil", price: 1299, mrp: 1899, cat: "🧪 Derm Pick", image: "/images/process1.png", hook: "Every oil made your skin greasy. This one gives you glass skin.", rating: "4.7", reviews: "960" },
              { id: "p4", name: "Vitamin C Booster", price: 1799, mrp: 2499, cat: "⚡ Low Stock", image: "/images/process3.png", hook: "Your skin looks tired even after 8hrs sleep? This fixes that.", rating: "4.9", reviews: "1.4K" }
            ].map((prod, idx) => (
              <div key={idx} className="min-w-75 md:min-w-120 group product-card snap-center product-focus">
                <div className="relative rounded-[2.5rem] md:rounded-[4rem] overflow-hidden aspect-square mb-8 bg-brand-nude border border-brand-pink/20 transition-all duration-700">
                  ...
                  <div className="absolute bottom-8 left-8 right-8 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700">
                    <Button variant="secondary" className="w-full justify-center text-white font-black" onClick={(e) => handleAddToCart(prod, e)}>Add to Bag — {prod.price}</Button>
                  </div>
                </div>
                <div className="px-4">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-sm font-black tracking-widest text-brand-pink">₹{typeof prod.price === 'number' ? prod.price.toLocaleString('en-IN') : prod.price}</span>
                      <span className="text-[9px] font-bold tracking-wider text-brand-dark/30 line-through">₹{typeof prod.mrp === 'number' ? prod.mrp.toLocaleString('en-IN') : prod.mrp}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[9px] font-black tracking-wider text-brand-dark/40">{prod.rating} ({prod.reviews} reviews)</span>
                  </div>
                  <p className="text-[10px] md:text-xs font-bold tracking-wide text-brand-dark/50 uppercase">{prod.hook}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="reveal-section py-5 md:py-10 bg-brand-dark text-brand-beige text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          ...
          <div className="flex flex-col items-center max-w-3xl mx-auto">
            <div className="w-full flex flex-col md:flex-row items-center gap-4 md:gap-0 bg-white/10 backdrop-blur-2xl rounded-[2.5rem] md:rounded-full p-3 border border-white/20 hover:border-brand-pink/50 transition-all group shadow-2xl">
              <input
                type="email"
                placeholder="YOUR BEST EMAIL"
                className="w-full bg-transparent px-10 py-5 md:py-0 text-white focus:outline-none placeholder-white/30 text-[10px] md:text-[12px] font-black tracking-[0.4em] uppercase"
              />
              <Button variant="secondary" className="w-full md:w-auto px-16 py-6 shadow-2xl hover:text-white cta-glow">
                Yes — Give Me 15% Off
              </Button>
            </div>
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-50 md:hidden transition-all duration-500 ${showStickyCta ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
        <div className="bg-brand-dark/95 backdrop-blur-xl border-t border-brand-pink/20 px-4 py-3 flex items-center justify-between gap-3">
          ...
          <Button variant="secondary" className="py-3 px-8 text-[9px] shadow-xl" onClick={(e) => handleAddToCart({ id: "p1", name: "Radiance Serum", price: 2499, image: "/images/serum.png" }, e)}>Shop Bestsellers</Button>
        </div>
      </div>
    </div>
  );
